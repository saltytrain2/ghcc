...
def find_cfiles(path: str) -> List[str]:
    r"""Find all paths to c files under the given directory.

    :param path: Path to the directory to scan.
    :return: A list of absolute paths to subdirectories that contain c files.
    """
    cfiles = []
...
class AssemblyResult(NamedTuple):
    success: bool
    asm_files: List[str]  # list of paths to ELF files
    error_type: Optional[AssemblyErrorType] = None
    captured_output: Optional[str] = None


def assemble_create_result(success: bool = False, asm_files: Optional[List[str]] = None,
                   error_type: Optional[AssemblyErrorType] = None,
                   captured_output: Optional[str] = None) -> AssemblyResult:
    if asm_files is None:
        asm_files = []
    return AssemblyResult(success, assembly_files=asm_files, error_type=error_type, captured_output=captured_output)
...
def _make_skeleton(directory: str, timeout: Optional[float] = None,
                   ...
                   verbose: bool = True,
                   *, make_fn,
                   check_file_fn: Callable[[str, str], bool] = _check_elf_fn) -> AssemblyResult:
